...
public class ProfileList {
    ...
    @PostConstruct
    public void init() {
        String rawList = System.getenv("PROFILE_LIST");
        if (rawList == null)
            throw new IllegalArgumentException("Missing conifiguration $PROFILE_LIST");
        profiles = Arrays.stream(rawList.split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .sorted()
